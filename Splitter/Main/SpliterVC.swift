...
//  VideoDownloader
//
//  Created by Junaid  Kamoka on 30/10/2023.
//
...
class SpliterVC: UIViewController {
            ...
            DispatchQueue.main.async {
                if let error = error {
                    ...
                    if hasMore {
                        ...
                        self.alert?.dismiss(animated: true, completion: {
                            self.showAlertController(message: "Split finished saved in Photos!", actions: [
...
extension SpliterVC: UIImagePickerControllerDelegate, UINavigationControllerDelegate {
...
extension SpliterVC: UIDocumentPickerDelegate {
...
extension SpliterVC: UITextFieldDelegate {
